	...
	tests := []struct {
	...
	}{
		{
			...
			prepare: func(client *mock.MockWebClient) {
				...
				client.EXPECT().Get("api/v1?lat=53.900000&lon=27.566700&appid=8rfne8&units=metric").Return(resp, nil)
			},
			lat:     53.9,
			lon:     27.5667,
			want:    21.5,
		...
		},
		{
			name: "client error",
			prepare: func(client *mock.MockWebClient) {
				client.EXPECT().Get("api/v1?lat=10.000000&lon=20.000000&appid=8rfne8&units=metric").Return(nil, errors.New("some error"))
			},
			lat:     10,
			lon:     20,
			wantErr: true,
		},
		{
			...
			prepare: func(client *mock.MockWebClient) {
				...
				client.EXPECT().Get("api/v1?lat=-1.250000&lon=100.000000&appid=8rfne8&units=metric").Return(resp, nil)
			},
			lat:     -1.25,
			lon:     100,
			wantErr: true,
		},
		{
			...
			prepare: func(client *mock.MockWebClient) {
				...
				client.EXPECT().Get("api/v1?lat=0.000000&lon=0.000000&appid=8rfne8&units=metric").Return(resp, nil)
			},
			lat:     0,
			lon:     0,
			wantErr: true,
