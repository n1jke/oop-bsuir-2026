...
type ExchangeRepository interface {
	AddExchange(ctx context.Context, exchange domain.ExchangeRequest) error
	...
	UpdateStatus(ctx context.Context, exchangeID uuid.UUID, status domain.ExchangeStatus) error
}

type OwnedBookRepository interface {
	AddBook(ctx context.Context, ownedBook domain.OwnedBook) (domain.OwnedBook, error)
...
type UserRepository interface {
	AddUser(ctx context.Context, login, password string) (uuid.UUID, error)
